 ...
 *
 * Modified by OpenMMLab: remove the TVM tensor include and exclude the
 * TVM-FFI wrapper from the standalone raw-pointer AOT build. The marlin_mm
 * compute implementation is unchanged.
 */
...
// The original SGLang source exposes a TVM-FFI TensorView wrapper below.
// LMDeploy calls marlin_mm through a raw-pointer pybind ABI, so the TVM
// wrapper is deliberately excluded from compilation.
#if 0

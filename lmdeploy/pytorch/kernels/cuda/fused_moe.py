...
@triton.jit
def _moe_reduce_add_fp32_kernel(
    hidden_states_ptr,
    weights_ptr,
    addend_ptr,
    routed_scratch_ptr,
    ...
    """Reduce routed experts, preserve BF16 rounding, then add in FP32."""
    pid = tl.program_id(0)
    num_n_split = tl.cdiv(N, BLOCK_N)
    mid = pid // num_n_split
    nid = pid % num_n_split
    ...
    offs_n = nid * BLOCK_N + tl.arange(0, BLOCK_N)
    weights_ptrs = weights_ptr + mid * stride_wm + offs_k * stride_wk
    hidden_ptrs = (hidden_states_ptr + mid * stride_hm
                   + offs_k[:, None] * stride_hk
                   + offs_n[None, :] * stride_hn)
    addend_ptrs = addend_ptr + mid * stride_am + offs_n * stride_an
    routed_scratch_ptrs = (routed_scratch_ptr + mid * stride_rm
                           + offs_n * stride_rn)
    out_ptrs = out_ptr + mid * stride_om + offs_n * stride_on
    ...
    routed = tl.sum(hidden * weights[:, None], axis=0)

    # Keep the materialized BF16 boundary from the unfused path.  A scratch
    # store/reload also prevents compiler reassociation from changing results.
    tl.store(routed_scratch_ptrs, routed, mask=mask_n)
    tl.debug_barrier()
    routed = tl.load(routed_scratch_ptrs, mask=mask_n).to(tl.float32)
    shared = tl.load(addend_ptrs, mask=mask_n).to(tl.float32)
    tl.store(out_ptrs, routed + shared, mask=mask_n)


def moe_reduce_add_fp32(
    hidden_states: torch.Tensor,
    topk_weights: torch.Tensor,
    addend: torch.Tensor,
) -> torch.Tensor:
    """Fuse routed reduction with the exact shared-expert FP32 add."""
    if hidden_states.device.type != 'cuda':
        raise ValueError('moe_reduce_add_fp32 requires CUDA tensors')
    if (topk_weights.device != hidden_states.device
            or addend.device != hidden_states.device):
        raise ValueError('moe_reduce_add_fp32 requires tensors on one device')
    if hidden_states.ndim != 3 or topk_weights.ndim != 2:
        raise ValueError('moe_reduce_add_fp32 expects 3D routes and 2D weights')
    if hidden_states.dtype != torch.bfloat16:
        raise ValueError('moe_reduce_add_fp32 requires BF16 routed/addend tensors')
    if addend.dtype != torch.bfloat16:
        raise ValueError('moe_reduce_add_fp32 requires BF16 routed/addend tensors')
    if topk_weights.dtype != torch.float32:
        raise ValueError('moe_reduce_add_fp32 requires FP32 router weights')
    if hidden_states.shape[:2] != topk_weights.shape:
        raise ValueError('moe_reduce_add_fp32 route and weight shapes differ')
    if addend.shape != (hidden_states.shape[0], hidden_states.shape[2]):
        raise ValueError('moe_reduce_add_fp32 addend shape differs')
    if not (hidden_states.is_contiguous() and topk_weights.is_contiguous()
            and addend.is_contiguous()):
        raise ValueError('moe_reduce_add_fp32 requires contiguous inputs')

    tokens, top_k, hidden_size = hidden_states.shape
    out = torch.empty(
        (tokens, hidden_size),
        dtype=torch.float32,
    ...
    num_warps = 1
    block_n = triton.cdiv(num_warps * 512,
                          hidden_states.element_size())
    grid = (tokens * triton.cdiv(hidden_size, block_n), )
    routed_scratch = torch.empty(
        (tokens, hidden_size),
        dtype=torch.bfloat16,
        device=hidden_states.device,
    )
    _moe_reduce_add_fp32_kernel[grid](
        hidden_states,
        topk_weights,
        addend,
        routed_scratch,
        ...
        stride_hn=hidden_states.stride(2),
        stride_wm=topk_weights.stride(0),
        stride_wk=topk_weights.stride(1),
        stride_am=addend.stride(0),
        stride_an=addend.stride(1),
        stride_rm=routed_scratch.stride(0),
        stride_rn=routed_scratch.stride(1),

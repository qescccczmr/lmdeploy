...
@pytest.mark.parametrize('tokens', [0, 1, 32, 257, 2048])
def test_moe_reduce_add_fp32_matches_materialized_boundary(tokens):
    ...
    if tokens:
        reference = moe_reduce(routes, weights, fp32_acc=True).float()
        reference += shared.float()
    else:
        reference = torch.empty_like(shared, dtype=torch.float32)
    result = moe_reduce_add_fp32(routes, weights, shared)
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason='requires CUDA')
def test_moe_reduce_add_fp32_cuda_graph_replay_is_exact():
    from lmdeploy.pytorch.kernels.cuda.fused_moe import (
        moe_reduce,
        moe_reduce_add_fp32,
    )
    ...
    moe_reduce_add_fp32(routes, weights, shared)
    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):
        graph_output = moe_reduce_add_fp32(routes, weights, shared)

    output_address = graph_output.data_ptr()
    for _ in range(2):
        routes.copy_(torch.randn_like(routes))
        weights.copy_(torch.rand_like(weights))
        weights.div_(weights.sum(dim=-1, keepdim=True))
        shared.copy_(torch.randn_like(shared))
        reference = (
            moe_reduce(routes, weights, fp32_acc=True).float()
            + shared.float())
        graph.replay()
        torch.cuda.synchronize()
        assert graph_output.data_ptr() == output_address
        assert torch.equal(graph_output, reference)
